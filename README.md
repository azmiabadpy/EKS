...
This project demonstrates an end-to-end DevOps workflow for deploying a containerized Node.js Task Management Application on Amazon EKS.
...
![Jenkins Pipeline](./Screenshots/eks_jenkins.png)
...
The Docker image is pushed to Docker Hub and used by the Kubernetes deployment.

### Docker Hub Image

![Docker Hub Image](./Screenshots/eks_dockerhub.png)
...
The application uses AWS credentials provided through the IAM role attached to the EKS worker node.
...
The actual secret file should be added to `.gitignore`:
...
* Implement pod-level IAM permissions
